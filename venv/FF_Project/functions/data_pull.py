...
class DataPulls():
            ...
            for x in range(0, len(i.scores)):
                for y in league.scoreboard(week=(x + 1)):
                    ...
                    if i.team_name == y.home_team.team_name:
                        ...
                        self.standings_data = FantastyFootball(team_name=team, opponent=opponent, week=week,
                                                      points_scored=points_scored,
                                                      points_against=points_against, mov=mov)
                        self.db_standings_commit(team, opponent, week, points_scored, points_against, mov)
    ...
    def db_standings_commit(self,team,opponent,week,points_scored,points_against,mov):
    ...
    def activity_pull(self):
        ...
        for i in range(0,(activity_len)):
            ...
            for x in range(0,(a_len)):
                ...
                self.activity_data = Activity_Tracker(team_name=team_name,action_taken=action_taken,player_name=player_name, readable_date=readable_date)
                self.db_activity_commit(team_name,action_taken,player_name,readable_date)

    def db_activity_commit(self,team_name,action_taken,player_name,readable_date):
        # check to make sure we don't add duplicated rows. Need to find more efficient way to handle
        conn = sqlite3.connect(
            'C:/Users/Brian/PycharmProjects/Fantasy_Football_Flask/venv/FF_Project/data.sqlite')
        conn.row_factory = sqlite3.Row
        exists = conn.execute("SELECT * "
                              ...
                              "AND action_taken=? "
                              "AND player_name=? "
                              "AND readable_date=?", (team_name, action_taken, player_name, readable_date, )).fetchone() is not None
        conn.close()
